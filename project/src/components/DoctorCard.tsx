...
const DoctorCard: React.FC<DoctorCardProps> = ({ doctor, variant = 'compact' }) => {
  return (
    <div className="bg-white rounded-lg shadow-lg overflow-hidden transition-all hover:shadow-xl border border-gray-100">
      <div className="p-4">
        <div className="flex flex-col md:flex-row gap-4">
          ...
          <div className="flex-1">
            ...
            {variant === 'full' && doctor.consultationFee && (
              <p className="mt-1 text-sm text-gray-600">
                <span className="font-medium">Phí tư vấn:</span> {doctor.consultationFee.toLocaleString()} VNĐ
              </p>
            )}
            
            <div className="mt-3 flex flex-wrap gap-2">
              <Link to={`/doctors/${doctor.id}`} className="bg-teal-100 text-teal-700 px-3 py-1 rounded-full text-xs font-medium hover:bg-teal-200 transition-colors">
                Chi tiết
              </Link>
      ...
      <div className="bg-gray-50 p-3 border-t border-gray-100">
        <div className="flex gap-2 justify-between">
          <Button variant="outline" size="sm" className="flex-1 hover:bg-teal-50">
